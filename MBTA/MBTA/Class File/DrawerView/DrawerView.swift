...
let kVerticalLeeway: CGFloat = 0

let kDefaultCornerRadius: CGFloat = 0

let kDefaultShadowRadius: CGFloat = 1.0

let kDefaultShadowOpacity: Float = 0.05

let kDefaultBackgroundEffect = UIBlurEffect(style: .dark)
...
@IBDesignable public class DrawerView: UIView {
    ...
    public var shadowOpacity: Float = kDefaultShadowOpacity {
        didSet {
            updateVisuals()
        }
    }
    ...
    convenience public init(withView view: UIView) {
        ...
        view.backgroundColor = #colorLiteral(red: 0.2941176471, green: 0.01568627451, blue: 0.01568627451, alpha: 1)
    ...
    private func updateBackgroundVisuals(_ backgroundView: UIVisualEffectView) {
        ...
        if #available(iOS 11.0, *) {
            ...
            let mask: CAShapeLayer = {
                ...
                let path = UIBezierPath(roundedRect: frame, byRoundingCorners: [.topLeft, .topRight], cornerRadii: CGSize(width: 0, height: 0))
                m.path = path.cgPath
                return m
            }()
